const CACHE='almoxarifado-v14';
const ASSETS=['./','./index.html','./manifest.json','./icons/icon-192.png','./icons/icon-512.png','./scripts/supplier-document-reader.js','./scripts/supplier-document-reader-fix.js','./scripts/supplier-document-reader-autosave.js','./scripts/supplier-document-reader-ie.js','./scripts/supplier-manual-fix.js'];
...
async function injectReader(response){
 ...
 const html=await response.text();if(html.includes(`./${MANUAL}`))return new Response(html,{status:response.status,statusText:response.statusText,headers:response.headers});
 const injected=html.replace('</body>',`<script src="./${READER}" defer></script><script src="./${FIX}" defer></script><script src="./${AUTOSAVE}" defer></script><script src="./${IE}" defer></script><script src="./${MANUAL}" defer></script></body>`);
