...
(() => {
  ...
  async function processReal(file){
    try{
      ...
      openSupplier('');
      setTimeout(()=>{
        field('sLegalName',data.razaoSocial);field('sTradeName',data.razaoSocial);field('sCnpj',data.cnpj);field('sIe',data.ie);field('sPhone',data.telefone);field('sAddress',data.endereco);field('sNumber',data.numero);field('sNeighborhood',data.bairro);field('sCity',data.cidade);field('sUf',data.uf);field('sCep',data.cep);field('sPurpose','Importação de documento — finalidade não informada');
        const overlay=$('#almoxReaderOverlay');if(overlay)overlay.classList.remove('open');if($('#modal'))$('#modal').classList.add('open');status('Dados extraídos e cadastro preparado.',100);
