...
(() => {
  ...
  function extract(raw){
    const text=String(raw||'').replace(/\u00a0/g,' '),lines=text.split(/\r?\n/).map(clean).filter(Boolean),out={razaoSocial:'',cnpj:'',cep:'',endereco:''};
    ...
    const bad=/^(CNPJ|CPF|INSCRI|IE\b|DANFE|DOCUMENTO|NOTA|ENDERE|CEP|FONE|TELEFONE|CHAVE|EMITENTE|DESTINAT)/i;
    // Nome é somente o texto identificado como razão social; não acrescentar endereço, CNPJ ou outros dados.
  ...
  async function processReal(file){
    try{
      ...
      openSupplier('');
      setTimeout(()=>{
        // Cadastro enxuto: somente os quatro dados exigidos.
        field('sLegalName',data.razaoSocial);field('sTradeName',data.razaoSocial);field('sCnpj',data.cnpj);field('sCep',data.cep);field('sAddress',data.endereco);
        // Os demais campos não recebem dados do documento.
        field('sIe','');field('sPhone','');field('sNumber','');field('sNeighborhood','');field('sCity','');field('sUf','');field('sPurpose','Importação de documento — finalidade não informada');
        const overlay=$('#almoxReaderOverlay');if(overlay)overlay.classList.remove('open');if($('#modal'))$('#modal').classList.add('open');status('Dados extraídos: razão social, CNPJ, CEP e endereço completo.',100);
